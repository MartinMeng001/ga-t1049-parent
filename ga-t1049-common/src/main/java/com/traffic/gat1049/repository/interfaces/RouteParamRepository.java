...
import com.traffic.gat1049.repository.entity.RouteParam;
 ...
 */
@Repository
public interface RouteParamRepository extends BaseMapper<RouteParam> {

    /**
     * 根据线路ID查询
     */
    @Select("SELECT * FROM route_param WHERE route_id = #{routeId}")
    RouteParam findByRouteId(@Param("routeId") String routeId);

    /**
     * 根据线路类型查询
     */
    @Select("SELECT * FROM route_param WHERE type = #{type} ORDER BY route_id")
    List<RouteParam> findByType(@Param("type") Integer type);
    ...
    List<RouteParam> findAllWithCompleteInfo();
    ...
    List<RouteParam> findBySubRegionId(@Param("subRegionId") String subRegionId);

    /**
     * 根据线路名称模糊查询
     */
    @Select("SELECT * FROM route_param WHERE route_name LIKE CONCAT('%', #{routeName}, '%') ORDER BY route_id")
    List<RouteParam> findByRouteNameLike(@Param("routeName") String routeName);
}
