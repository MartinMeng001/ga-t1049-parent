 ...
 */
@Repository
public interface DayPlanParamRepository extends BaseMapper<DayPlanParamEntity> {

    /**
     * 根据路口ID查询所有日计划
     */
    @Select("SELECT * FROM day_plan_param WHERE cross_id = #{crossId} ORDER BY day_plan_no")
    List<DayPlanParamEntity> findByCrossId(@Param("crossId") String crossId);

    /**
     * 根据路口ID和日计划号查询
     */
    @Select("SELECT * FROM day_plan_param WHERE cross_id = #{crossId} AND day_plan_no = #{dayPlanNo}")
    DayPlanParamEntity findByCrossIdAndDayPlanNo(@Param("crossId") String crossId, @Param("dayPlanNo") Integer dayPlanNo);
    ...
    List<DayPlanParamEntity> findWithPeriods(@Param("crossId") String crossId);
    ...
    DayPlanParamEntity findCurrentActivePlan(@Param("crossId") String crossId, @Param("currentTime") LocalTime currentTime);
}