 ...
 */
@Repository
public interface PedestrianParamRepository extends BaseMapper<PedestrianParamEntity> {

    /**
     * 根据路口ID查询所有人行横道
     */
    @Select("SELECT * FROM pedestrian_param WHERE cross_id = #{crossId} ORDER BY pedestrian_no")
    List<PedestrianParamEntity> findByCrossId(@Param("crossId") String crossId);

    /**
     * 根据路口ID和人行横道号查询
     */
    @Select("SELECT * FROM pedestrian_param WHERE cross_id = #{crossId} AND pedestrian_no = #{pedestrianNo}")
    PedestrianParamEntity findByCrossIdAndPedestrianNo(@Param("crossId") String crossId, @Param("pedestrianNo") Integer pedestrianNo);

    /**
     * 根据方向查询人行横道
     */
    @Select("SELECT * FROM pedestrian_param WHERE cross_id = #{crossId} AND direction = #{direction} ORDER BY pedestrian_no")
    List<PedestrianParamEntity> findByDirection(@Param("crossId") String crossId, @Param("direction") String direction);

    /**
     * 查询有按钮的人行横道
     */
    @Select("SELECT * FROM pedestrian_param WHERE cross_id = #{crossId} AND has_button = 1 ORDER BY pedestrian_no")
    List<PedestrianParamEntity> findWithButton(@Param("crossId") String crossId);

    /**
     * 查询有语音提示的人行横道
     */
    @Select("SELECT * FROM pedestrian_param WHERE cross_id = #{crossId} AND has_voice = 1 ORDER BY pedestrian_no")
    List<PedestrianParamEntity> findWithVoice(@Param("crossId") String crossId);
    ...
    List<PedestrianParamEntity> findBySignalGroup(@Param("crossId") String crossId, @Param("signalGroupNo") Integer signalGroupNo);
}
