...
import com.traffic.gat1049.repository.entity.LaneParam;
 ...
 */
@Repository
public interface LaneParamRepository extends BaseMapper<LaneParam> {

    /**
     * 根据路口ID查询所有车道
     */
    @Select("SELECT * FROM lane_param WHERE cross_id = #{crossId} ORDER BY lane_no")
    List<LaneParam> findByCrossId(@Param("crossId") String crossId);

    /**
     * 根据路口ID和车道号查询
     */
    @Select("SELECT * FROM lane_param WHERE cross_id = #{crossId} AND lane_no = #{laneNo}")
    LaneParam findByCrossIdAndLaneNo(@Param("crossId") String crossId, @Param("laneNo") Integer laneNo);

    /**
     * 根据进口方向查询车道
     */
    @Select("SELECT * FROM lane_param WHERE cross_id = #{crossId} AND entrance = #{entrance} ORDER BY lane_no")
    List<LaneParam> findByEntrance(@Param("crossId") String crossId, @Param("entrance") String entrance);

    /**
     * 根据车道转向属性查询
     */
    @Select("SELECT * FROM lane_param WHERE cross_id = #{crossId} AND movement = #{movement} ORDER BY lane_no")
    List<LaneParam> findByMovement(@Param("crossId") String crossId, @Param("movement") Integer movement);

    /**
     * 查询机动车道
     */
    @Select("SELECT * FROM lane_param WHERE cross_id = #{crossId} AND type = 1 ORDER BY lane_no")
    List<LaneParam> findVehicleLanes(@Param("crossId") String crossId);

    /**
     * 查询非机动车道
     */
    @Select("SELECT * FROM lane_param WHERE cross_id = #{crossId} AND type = 2 ORDER BY lane_no")
    List<LaneParam> findNonVehicleLanes(@Param("crossId") String crossId);
    ...
    List<LaneParam> findBySignalGroup(@Param("crossId") String crossId, @Param("signalGroupNo") Integer signalGroupNo);
}
