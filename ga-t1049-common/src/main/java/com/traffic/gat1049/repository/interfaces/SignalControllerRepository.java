...
import com.traffic.gat1049.repository.entity.SignalControllerParam;
 ...
 */
@Repository
public interface SignalControllerRepository extends BaseMapper<SignalControllerParam> {

    /**
     * 根据信号机ID查询
     */
    @Select("SELECT * FROM signal_controller WHERE signal_controller_id = #{signalControllerId}")
    SignalControllerParam findBySignalControllerId(@Param("signalControllerId") String signalControllerId);
    ...
    List<SignalControllerParam> findAllWithCompleteInfo();

    /**
     * 根据供应商查询
     */
    @Select("SELECT * FROM signal_controller WHERE supplier = #{supplier} ORDER BY signal_controller_id")
    List<SignalControllerParam> findBySupplier(@Param("supplier") String supplier);

    /**
     * 根据类型查询
     */
    @Select("SELECT * FROM signal_controller WHERE type = #{type} ORDER BY signal_controller_id")
    List<SignalControllerParam> findByType(@Param("type") String type);
    ...
    List<SignalControllerParam> findByCrossId(@Param("crossId") String crossId);
    ...
    List<SignalControllerParam> findBySystemId(@Param("systemId") String systemId);

    /**
     * 根据IP地址查询
     */
    @Select("SELECT * FROM signal_controller WHERE ip = #{ip}")
    List<SignalControllerParam> findByIp(@Param("ip") String ip);
}
