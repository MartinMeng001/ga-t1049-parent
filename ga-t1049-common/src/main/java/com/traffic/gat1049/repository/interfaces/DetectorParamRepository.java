...
import com.traffic.gat1049.repository.entity.DetectorParam;
 ...
 */
@Repository
public interface DetectorParamRepository extends BaseMapper<DetectorParam> {

    /**
     * 根据路口ID查询所有检测器
     */
    @Select("SELECT * FROM detector_param WHERE cross_id = #{crossId} ORDER BY detector_no")
    List<DetectorParam> findByCrossId(@Param("crossId") String crossId);

    /**
     * 根据路口ID和检测器号查询
     */
    @Select("SELECT * FROM detector_param WHERE cross_id = #{crossId} AND detector_no = #{detectorNo}")
    DetectorParam findByCrossIdAndDetectorNo(@Param("crossId") String crossId, @Param("detectorNo") Integer detectorNo);

    /**
     * 根据检测器类型查询
     */
    @Select("SELECT * FROM detector_param WHERE cross_id = #{crossId} AND type = #{type} ORDER BY detector_no")
    List<DetectorParam> findByType(@Param("crossId") String crossId, @Param("type") Integer type);

    /**
     * 根据检测功能查询
     */
    @Select("SELECT * FROM detector_param WHERE cross_id = #{crossId} AND function = #{function} ORDER BY detector_no")
    List<DetectorParam> findByFunction(@Param("crossId") String crossId, @Param("function") Integer function);
    ...
    List<DetectorParam> findByLane(@Param("crossId") String crossId, @Param("laneNo") Integer laneNo);
    ...
    List<DetectorParam> findByPedestrian(@Param("crossId") String crossId, @Param("pedestrianNo") Integer pedestrianNo);

    /**
     * 查询感应检测器
     */
    @Select("SELECT * FROM detector_param WHERE cross_id = #{crossId} AND function IN (1, 2, 3) ORDER BY detector_no")
    List<DetectorParam> findInductiveDetectors(@Param("crossId") String crossId);
}
