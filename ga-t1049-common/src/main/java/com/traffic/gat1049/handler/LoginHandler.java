...
import com.traffic.gat1049.model.dto.response.LoginResponse;
...
public class LoginHandler extends AbstractProtocolHandler {
    ...
    private Message createLoginResponse(Message request, SessionManager.LoginResult loginResult)
        ...
        if (loginResult.isSuccess()) {
            ...
            response.setOnlineUsers(sessionManager.getOnlineSessionCount());
            response.setTimestamp();

            Message responseMessage = createSuccessResponse(request, response);
            // 重要：将token设置到消息头部
            responseMessage.setToken(loginResult.getToken());
