...
CREATE
DATABASE IF NOT EXISTS traffic_signal_control
...
-- 54. 供应商字典表 (SupplierDictionary)
CREATE TABLE supplier_dictionary
(
    supplier_id            VARCHAR(20) PRIMARY KEY COMMENT '供应商编号',
    supplier_short_name_en VARCHAR(50) COMMENT '供应商英文简称',
    supplier_full_name_en  VARCHAR(100) COMMENT '供应商英文全称',
    supplier_name_cn       VARCHAR(100) NOT NULL COMMENT '供应商中文名',
    created_time           TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_time           TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE INDEX uk_supplier_name_cn (supplier_name_cn)
) COMMENT = '供应商字典表';
...
FROM sys_info si
         ...
         LEFT JOIN (SELECT scr.system_id,
                    ...
                    GROUP BY scr.system_id) cross_list ON si.system_id = cross_list.system_id

         LEFT JOIN (SELECT ssrr.system_id,
                    ...
                    GROUP BY ssrr.system_id) sub_region_list ON si.system_id = sub_region_list.system_id

         LEFT JOIN (SELECT srr.system_id,
                    ...
                    GROUP BY srr.system_id) route_list ON si.system_id = route_list.system_id

         LEFT JOIN (SELECT srgr.system_id,
                    ...
                    GROUP BY srgr.system_id) region_list ON si.system_id = region_list.system_id

         LEFT JOIN (SELECT sscr.system_id,
                    ...
                    GROUP BY sscr.system_id) controller_list ON si.system_id = controller_list.system_id
...
FROM region_param rp
         ...
         LEFT JOIN (SELECT rsr.region_id,
                           GROUP_CONCAT(rsr.sub_region_id ORDER BY rsr.sub_region_id SEPARATOR ',') as sub_region_ids,
                           COUNT(*)                                                                 as sub_region_count
                    FROM region_sub_region rsr
                    GROUP BY rsr.region_id) sub_region_list ON rp.region_id = sub_region_list.region_id

         LEFT JOIN (SELECT rc.region_id,
                           GROUP_CONCAT(rc.cross_id ORDER BY rc.cross_id SEPARATOR ',') as cross_ids,
                           COUNT(*)                                                     as cross_count
                    FROM region_cross rc
                    GROUP BY rc.region_id) cross_list ON rp.region_id = cross_list.region_id;
...
FROM route_param rp
         ...
         LEFT JOIN (SELECT rc.route_id,
                           CONCAT('[', GROUP_CONCAT(
                                   CONCAT('{"CrossID":"', rc.cross_id, '","Distance":', rc.distance, ',"OrderSeq":',
                                          rc.order_seq, '}') ORDER BY rc.order_seq SEPARATOR ','
                                       ), ']') as route_cross_data,
                           COUNT(*)            as cross_count
                    FROM route_cross rc
                    GROUP BY rc.route_id) route_cross_list ON rp.route_id = route_cross_list.route_id

         LEFT JOIN (SELECT rsr.route_id,
                           GROUP_CONCAT(rsr.sub_region_id ORDER BY rsr.sub_region_id SEPARATOR ',') as sub_region_ids,
                           COUNT(*)                                                                 as sub_region_count
                    FROM route_sub_region rsr
                    GROUP BY rsr.route_id) sub_region_list ON rp.route_id = sub_region_list.route_id;

-- 4. 子区参数完整信息视图 (SubRegionParam)
CREATE VIEW v_sub_region_param_complete AS
SELECT srp.sub_region_id,
       srp.sub_region_name,
       srp.created_time,
       srp.updated_time,
...
FROM sub_region_param srp
         ...
         LEFT JOIN (SELECT src.sub_region_id,
                           GROUP_CONCAT(src.cross_id ORDER BY src.cross_id SEPARATOR ',') as cross_ids,
                           COUNT(*)                                                       as cross_count
                    FROM sub_region_cross src
                    GROUP BY src.sub_region_id) cross_list ON srp.sub_region_id = cross_list.sub_region_id
         ...
         LEFT JOIN (SELECT src.sub_region_id,
                    ...
                    GROUP BY src.sub_region_id) key_cross_list ON srp.sub_region_id = key_cross_list.sub_region_id;

-- 5. 信号机参数完整信息视图 (SignalController)
CREATE VIEW v_signal_controller_complete AS
SELECT sc.signal_controller_id,
       sc.supplier,
       sc.type,
       sc.id_code,
       sc.comm_mode,
...
FROM signal_controller sc
         ...
         LEFT JOIN (SELECT scc.signal_controller_id,
                    ...
                    GROUP BY scc.signal_controller_id) cross_list
                   ON sc.signal_controller_id = cross_list.signal_controller_id;
...
FROM cross_param cp
         ...
         LEFT JOIN (SELECT cross_id,
                    ...
                    GROUP BY cross_id) pedestrian_list ON cp.cross_id = pedestrian_list.cross_id
         ...
         LEFT JOIN (SELECT cross_id,
                    ...
                    GROUP BY cross_id) lamp_group_list ON cp.cross_id = lamp_group_list.cross_id
         ...
         LEFT JOIN (SELECT cross_id,
                    ...
                    GROUP BY cross_id) signal_group_list ON cp.cross_id = signal_group_list.cross_id

         LEFT JOIN (SELECT cross_id,
                           GROUP_CONCAT(stage_no ORDER BY stage_no SEPARATOR ',') as stage_no_list,
                           COUNT(*)                                               as stage_count
                    FROM stage_param
                    GROUP BY cross_id) stage_list ON cp.cross_id = stage_list.cross_id

         LEFT JOIN (SELECT cross_id,
                           GROUP_CONCAT(plan_no ORDER BY plan_no SEPARATOR ',') as plan_no_list,
                           COUNT(*)                                             as plan_count
                    FROM plan_param
                    GROUP BY cross_id) plan_list ON cp.cross_id = plan_list.cross_id

         LEFT JOIN (SELECT cross_id,
                           GROUP_CONCAT(day_plan_no ORDER BY day_plan_no SEPARATOR ',') as day_plan_no_list,
                           COUNT(*)                                                     as day_plan_count
                    FROM day_plan_param
                    GROUP BY cross_id) day_plan_list ON cp.cross_id = day_plan_list.cross_id

         LEFT JOIN (SELECT cross_id,
                           GROUP_CONCAT(schedule_no ORDER BY schedule_no SEPARATOR ',') as schedule_no_list,
                           COUNT(*)                                                     as schedule_count
                    FROM schedule_param
                    GROUP BY cross_id) schedule_list ON cp.cross_id = schedule_list.cross_id;
...
-- 系统最新状态
         LEFT JOIN (SELECT ss.system_id,
                           ss.value as  sys_status,
                           ss.time  as  last_status_time,
                           ROW_NUMBER() OVER (PARTITION BY ss.system_id ORDER BY ss.time DESC) as rn
                    FROM sys_state ss) latest_sys_state
                   ON si.system_id = latest_sys_state.system_id AND latest_sys_state.rn = 1
         ...
         LEFT JOIN (SELECT system_id,
                    ...
                    FROM (SELECT system_id,
                                 COUNT(*) as cross_count,
                                 0        as sub_region_count,
                                 0        as route_count,
                                 0        as region_count,
                                 0        as controller_count
                          FROM sys_cross_relation
                          WHERE is_active = 1
                          GROUP BY system_id
                          UNION ALL
                          SELECT system_id, 0, COUNT(*), 0, 0, 0
                          FROM sys_sub_region_relation
                          WHERE is_active = 1
                          GROUP BY system_id
                          UNION ALL
                          SELECT system_id, 0, 0, COUNT(*), 0, 0
                          FROM sys_route_relation
                          WHERE is_active = 1
                          GROUP BY system_id
                          UNION ALL
                          SELECT system_id, 0, 0, 0, COUNT(*), 0
                          FROM sys_region_relation
                          WHERE is_active = 1
                          GROUP BY system_id
                          UNION ALL
                          SELECT system_id, 0, 0, 0, 0, COUNT(*)
                          FROM sys_signal_controller_relation
                          WHERE is_active = 1
                          GROUP BY system_id) combined_stats
                    GROUP BY system_id) stats ON si.system_id = stats.system_id

         LEFT JOIN (SELECT scr.system_id,
                           COUNT(DISTINCT CASE
                                              WHEN cs.created_time >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
                                                  THEN scr.cross_id END)              as active_crosses,
                           COUNT(DISTINCT CASE
                                              WHEN sce.occur_time >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
                                                  THEN sscr.signal_controller_id END) as active_controllers
                    FROM sys_cross_relation scr
                             ...
                             LEFT JOIN signal_controller_error sce
                                       ON sscr.signal_controller_id = sce.signal_controller_id
                    WHERE scr.is_active = 1
                    GROUP BY scr.system_id) active_stats ON si.system_id = active_stats.system_id;
...
-- 路口形状字典
INSERT INTO data_dictionary (dict_type, dict_code, dict_name, dict_value, sort_order)
VALUES ('cross_feature', '10', '行人过街', '行人过街', 1),
...
-- 路口等级字典
INSERT INTO data_dictionary (dict_type, dict_code, dict_name, dict_value, sort_order)
VALUES ('cross_grade', '11', '一级', '主干路与主干路相交交叉口', 1),
...
-- 控制方式字典
INSERT INTO data_dictionary (dict_type, dict_code, dict_name, dict_value, sort_order)
VALUES ('ctrl_mode', '00', '撤销或恢复自主', '撤销或恢复自主', 1),
...
-- 线路类型字典
INSERT INTO data_dictionary (dict_type, dict_code, dict_name, dict_value, sort_order)
VALUES ('route_type', '1', '协调干线', '协调干线', 1),
...
-- 检测器类型字典
INSERT INTO data_dictionary (dict_type, dict_code, dict_name, dict_value, sort_order)
VALUES ('detector_type', '1', '线圈', '线圈', 1),
...
-- 车道转向属性字典
INSERT INTO data_dictionary (dict_type, dict_code, dict_name, dict_value, sort_order)
VALUES ('lane_movement', '11', '直行', '直行', 1),
...
-- 信号机故障类型字典
INSERT INTO data_dictionary (dict_type, dict_code, dict_name, dict_value, sort_order)
VALUES ('error_type', '1', '灯输出故障', '灯输出故障', 1),
...
WHERE csgs.cross_id = p_cross_id
  AND csgs.lamp_status_time = (SELECT MAX(lamp_status_time)
...
CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    ...
    SET
@cleanup_date = DATE_SUB(CURDATE(), INTERVAL 3 MONTH);
...
BEGIN
    DECLARE
v_partition_name VARCHAR(20);
    DECLARE
v_partition_value INT;
    DECLARE
v_sql TEXT;

    SET
v_partition_name = CONCAT('p', p_year, LPAD(p_month, 2, '0'));
    SET
v_partition_value = p_year * 100 + p_month + 1;
